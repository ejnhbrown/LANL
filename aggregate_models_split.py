...
def train_model(X=X_train_scaled.iloc[trainlist], X_20=X_train_scaled.iloc[testlist],X_test=X_test_scaled, y=y_tr.iloc[trainlist], params=None, folds=folds, model_type='lgb', model=None):
    oof = np.zeros(len(X))
    prediction = np.zeros(len(X_test))
    pred20 = np.zeros(len(X_20))
    scores = []
    ...
    for fold_n, (train_index, valid_index) in enumerate(folds.split(X)):
        ...
        if model_type == 'lgb':
            ...
            y_20 = model.predict(X_20, num_iteration=model.best_iteration_)
        ...
        if model_type == 'xgb':
            ...
            y_20 = model.predict(xgb.DMatrix(X_20, feature_names=X.columns), ntree_limit=model.best_ntree_limit)
        ...
        if model_type == 'sklearn':
            ...
            y_20 = model.predict(X_20).reshape(-1, )
    ...
    return oof, prediction, pred20, CVscore
...
print('xgb:')
oof_xgb, prediction_xgb, pred20_xgb, CVscore_xgb = train_model(params=xgb_params, model_type='xgb')
np.save('agg_data/xgb/80/xgb.npy',oof_xgb)
np.save('agg_data/xgb/20/xgb.npy',pred20_xgb)
np.save('agg_data/xgb/test/xgb.npy',prediction_xgb)
print('lgb:')
oof_lgb, prediction_lgb, pred20_lgb, CVscore_lgb = train_model(params=lgb_params, model_type='lgb')
np.save('agg_data/lgb/80/lgb.npy',oof_lgb)
np.save('agg_data/lgb/20/lgb.npy',pred20_lgb)
np.save('agg_data/lgb/test/lgb.npy',prediction_lgb)
print('svr:')
oof_svr, prediction_svr, pred20_svr, CVscore_svr = train_model(params=None, model_type='sklearn', model=NuSVR_model)
np.save('agg_data/svr/80/svr.npy',oof_svr)
np.save('agg_data/svr/20/svr.npy',pred20_svr)
np.save('agg_data/svr/test/svr.npy',prediction_svr)
print('svr2:')
oof_svr2, prediction_svr2, pred20_svr2, CVscore_svr2 = train_model(params=None, model_type='sklearn', model=NuSVR2_model)
np.save('agg_data/svr2/80/svr2.npy',oof_svr2)
np.save('agg_data/svr2/20/svr2.npy',pred20_svr2)
np.save('agg_data/svr2/test/svr2.npy',prediction_svr2)
...
print('kr:')
oof_kr, prediction_kr, pred20_kr, CVscore_kr = train_model(params=None, model_type='sklearn', model=KR_model)
np.save('agg_data/kr/80/kr.npy',oof_kr)
np.save('agg_data/kr/20/kr.npy',pred20_kr)
np.save('agg_data/kr/test/kr.npy',prediction_kr)
# blend:
